...
def run_case(case: dict[str, object]) -> list[str]:
    ...
    for index, candidate in enumerate(input_doc.get("candidates", []), start=1):
        if not isinstance(candidate, dict) or not candidate.get("screenshot"):
            continue
        screenshot_name = f"candidate-{index:03d}.png"
        (case_input_root / screenshot_name).write_bytes(SCREENSHOT_PNG)
        candidate["screenshot"] = screenshot_name
...
def run_validator_fail_closed_case() -> list[str]:
    errors: list[str] = []
    source_run = OUTPUT_ROOT / "eval-matching-candidates-within-cap"
    run_dir = OUTPUT_ROOT / "validator-non-object"
    shutil.copytree(source_run, run_dir)
    (run_dir / "artifacts" / "candidates.json").write_text("[]\n", encoding="utf-8")
    completed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "validation" / "validate_price_candidate_discovery.py"),
            str(run_dir),
        ],
        cwd=ROOT,
        text=True,
    ...
    if completed.returncode == 0:
        errors.append("validator-non-object: malformed candidate evidence passed validation")
    elif "json_object_required:artifacts/candidates.json" not in completed.stdout:
        errors.append(f"validator-non-object: missing fail-closed error: {completed.stderr.strip()}")
    return errors
...
def main() -> int:
    ...
    all_errors.extend(run_validator_fail_closed_case())
